%% mainFieldDRTConnectionsNHIG - find DRT connected components for the whole field
...
%                data structures according to NHIG method.
%
%   requirements:
...
[PHI,KX,KY,KZ,KN,PHIZ,~,~,~] = loadMatFiles;

load('../mat/FZI_NHIG_m1.5.mat')
load('../mat/RQI_NHIG_m1.5.mat')
load('../mat/DRT_NHIG_m1.5.mat')
...
for m = 1:length(drt)
    ...
    if csv_flag == true
        ...
        % preparing csv file
        fname = strcat('../csv/Table-Field','_DRT_NHIG_',num2str( drt(m) ),'.csv');    
        dlmwrite(fname,txt,'');        
    ...
    for idcomp = 1:ncomp
        ...
        if (csv_flag == true) && (compSizes(idcomp) >= nofsv)              
            % preparing csv file
            fname = strcat('../csv/Table-Cluster_',num2str(idcomp),'_DRT_NHIG_',num2str( drt(m) ),'.csv');        
            dlmwrite(fname,txt,'');       
    ...
    save( strcat('../mat/DRT_NHIG_',num2str( drt(m) ),'.mat'),'drtSt'); % saving
...
% export to VTK
if vtk_flag == true
    saveVtkCellCentered(DRT,'DRT_NHIG_Field','DRT_NHIG_Voxel','DRT_NHIG_Point');
end
